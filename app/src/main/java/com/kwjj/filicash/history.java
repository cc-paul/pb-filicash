...
public class history extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lnSent.setOnClickListener(
                view -> {
                    vwReceived.setVisibility(View.INVISIBLE);
                    vwSent.setVisibility(View.VISIBLE);
                    loadHistory("out");
                }
        );
    ...
    public void loadHistory(String flow) {
        ...
        String historyAPI = application.history;
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, historyAPI,
                response -> {
                    try {
                        ...
                        if (error){
                            Toast.makeText(history.this, "Unable to get data from server", Toast.LENGTH_LONG).show();
                        } else {
                            ...
                            for (Integer i = 0; i < arrHistory.length(); i++) {
                                ...
                                if (current_obj.getString("flow").equals(flow)) {
